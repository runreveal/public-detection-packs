...
WHERE (receivedAt > {from:DateTime})
  ...
  AND (lower(JSONExtractString(arrayFirst(x -> JSONExtractString(x, 'name') = 'NEW_VALUE', JSONExtractArrayRaw(event, 'parameters')), 'value')) = 'true')
  AND (JSONExtractString(arrayFirst(x -> JSONExtractString(x, 'name') = 'SETTING_NAME', JSONExtractArrayRaw(event, 'parameters')), 'value') = 'Password Management - Enable password reuse')
; 