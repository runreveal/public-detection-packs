...
WHERE (NOT (`client.appName` IS NULL)) AND (client.appName != '') AND (client.platformName != '') AND (user NOT LIKE '%1passwordserviceaccounts.com') AND ((receivedAt >= {from:DateTime}) AND (receivedAt <= {to:DateTime})) AND (previousClients.user != '') AND (NOT has(previousClients.clients, (`client.appName`, `client.platformName`)))
;
